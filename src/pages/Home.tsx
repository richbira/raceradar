...
export default function Home() {
  ...
  const { races, loading, error } = useRaces(); // fetch di tutte le gare, poi filtriamo in frontend (per ora va bene, se diventano troppe gare si può implementare la paginazione anche a livello di query)
  ...
  const selectedDistance = (searchParams.get("distance") ?? "all") as
    | RaceDistance
    | "all";
  ...
  const filtered = useMemo(() => {
    return races.filter((race) => {
      ...
      const matchDistance =
        selectedDistance === "all" || race.distances.includes(selectedDistance);
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 flex flex-col gap-4">
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex flex-col items-end gap-1">
            <span className="text-xs text-black-400 hidden sm:inline">
              La tua gara non c'è?
            </span>
            <button
      ...
      <div className="max-w-4xl mx-auto px-4 pb-10">
        ...
        {filtered.length === 0 && (
          <div className="text-center text-gray-400 py-12 flex flex-col items-center gap-4">
            <p>Nessuna gara trovata con questi filtri.</p>
            <button
              onClick={() => navigate("/proponi-gara")}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {paginated.map((race, index) => (
            <RaceCard
              key={`${race.id}-${search}-${selectedType}-${selectedDistance}`}
              race={race}
              index={index}
            />
          ))}
